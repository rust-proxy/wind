...
impl InboundCallback for DirectCallback {
	...
	async fn handle_udpstream(&self, stream: UdpStream) -> eyre::Result<()> {
		...
		// Task: read responses from the real target and send them back through the
		// tunnel
...
async fn test_tuic_udp_proxy() -> eyre::Result<()> {
	...
	tokio::spawn(async move {
		let _ = client_for_udp.handle_udp(client_stream, None::<TuicOutbound>).await;
	});
	...
	tracing::info!(
		"UDP echo test passed: sent {} bytes, received {} bytes",
		test_payload.len(),
		pkt.payload.len()
	);
	...
	tokio::spawn(async move {
		...
		loop {
			match echo_clone.recv_from(&mut buf).await {
				Ok((n, peer)) => {
					let _ = echo_clone.send_to(&buf[..n], peer).await;
				}
				Err(_) => break,
			}
		}
